...
// so it is enabled and disabled on its own instead of riding the Codex apply
// path. The base URL is printed; the caller key inside it never is.
async function handleClaudeCode(action) {
  const { claudeCodeSnapshot, disableClaudeCodeTarget, enableClaudeCodeTarget } = await import(
    "./claude-code-integration.mjs"
  );
  const { redactCallerUrl } = await import("./caller-auth.mjs");
  const redact = (result) => ({
    ...result,
    ...(result.baseUrl ? { baseUrl: redactCallerUrl(result.baseUrl) } : {}),
  });
  if (!action || action === "status") {
    process.stdout.write(`${JSON.stringify(redact(claudeCodeSnapshot()))}\n`);
    return;
  }
  if (action === "enable") {
    process.stdout.write(`${JSON.stringify(redact(enableClaudeCodeTarget()))}\n`);
    return;
  }
  if (action === "disable") {
    process.stdout.write(`${JSON.stringify(redact(disableClaudeCodeTarget()))}\n`);
    return;
  }
  throw new Error("Usage: control claude-code status|enable|disable");
}
...
if (args.includes("--probe")) {
  ...
  handleTray(args[1]);
} else if (args[0] === "claude-code") {
  await handleClaudeCode(args[1]);
