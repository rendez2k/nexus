...
// Kept in step with the `local` provider's displayName in config/local/, which
// this module cannot import without closing a cycle through model-registry.mjs.
const LOCAL_PROVIDER_DISPLAY_NAME = "Local (Ollama)";
  ...
  const entries = publishable.map((tag, index) => {
    ...
    return {
      ...userModelEntry({
        ...
        providerDisplayName: LOCAL_PROVIDER_DISPLAY_NAME,
        upstreamId: tag,
